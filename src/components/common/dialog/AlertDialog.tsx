...
const AlertDialog: React.FC<AlertDialogProps> = ({
	...
	return (
		...
		>
			<DialogTitle className='flex items-center justify-between border-0 border-b-[0.5px] border-solid border-gray p-4'>
			...
			{mode === DialogMode.Confirm ? (
				<DialogActions className='flex items-center gap-3 p-4 [&_.MuiButtonBase-root]:m-0'>
					<ActionButton
						...
						isOutlinedButton
					/>
					<ActionButton title={confirmTitle ?? t('CONFIRM')} onClick={onConfirm} loading={loading} />
				</DialogActions>
			) : (
				<DialogActions className='p-4'>
					<ActionButton title={confirmTitle ?? t('CLOSE')} onClick={onConfirm} loading={loading} />
				</DialogActions>
			)}
		</Dialog>
	)
